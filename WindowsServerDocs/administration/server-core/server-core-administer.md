...
3. 运行以下 cmdlet 以设置静态 IP 地址:
   ...
   其中：
   - **InterfaceIndex**是步骤2中**IfIndex**的值。 (在我们的示例中为 12)
   - **IPAddress**是要设置的静态 IP 地址。 (在我们的示例中, 191.0.2.2)
   - **PrefixLength**是所设置的 IP 地址的前缀长度 (子网掩码的另一种形式)。 (对于我们的示例, 为 24)
...
|               向本地计算机添加组               |                                                                                                                                                                                                 **net localgroup \<组名称\> /add**                                                                                                                                                                                                  |
...
|                 更改静态 IP 地址                 | **ipconfig/all** <br>记录相关信息或将其重定向到文本文件 (**ipconfig/all > ipconfig .txt**)。<br>**netsh interface ipv4 show interface**<br>验证是否有接口列表。<br>**netsh interface ipv4 set address name \<ID from interface list\> source = static address =\<首选 IP 地址\>网关\<= 网关地址\>**<br>运行**ipconfig/all** , 验证 DHCP 是否已启用设置为 "**否**"。 |
|                   设置静态 DNS 地址。                   |   <strong>netsh interface ipv4 add dnsserver name =\<网络接口卡\>地址\<的名称或 ID = 主 DNS 服务器\>索引的 IP 地址 = 1 <br></strong>netsh interface ipv4 add dnsserver name =\<辅助 dns 服务器\>地址\<的名称 = 辅助 dns 服务器\>索引的 IP 地址 = 2\*\* <br> 根据需要重复此步骤以添加其他服务器。<br>运行**ipconfig/all**以验证地址是否正确。   |
| 从静态 IP 地址更改为 DHCP 提供的 IP 地址 |                                                                                                                                      **netsh 接口 ipv4 设置地址名称 =\<本地系统\>源的 IP 地址 = DHCP** <br>运行**ipconfig/all** , 验证是否已将 DCHP 设置为 **"是"** 。                                                                                                                                      |
|                      输入产品密钥                      |                                                                                                                                                                                                   **slmgr.vbs – ipk \<产品密钥\>**                                                                                                                                                                                                    |
|                  本地激活服务器                  |                                                                                                                                                                                                           **slmgr.vbs-ato**                                                                                                                                                                                                            |
|                 远程激活服务器                  |                                            **cscript slmgr.vbs-ipk \<产品密钥\<\>服务器名称\<用户名密码\>\>\<\>** <br>**\<cscript slmgr.vbs-ato servername \<\>用户名password\> \<\>** <br>通过运行**cscript slmgr.vbs**获取计算机的 GUID-已 <br> 运行**cscript slmgr.vbs-dli \<GUID\>** <br>验证 "许可证状态" 是否设置为 "已**许可 (已激活)** "。                                             |
...
|                      列出安装的更新                       |                                                                                                                            **systeminfo**                                                                                                                            |
|                         删除更新                          |                                 **展开/f:\* \<update-help\>c:\test** <br>导航到 c:\test\, 并\<在\>文本编辑器中打开 "update-help"。<br>将**Install**替换为**Remove** , 并保存该文件。<br>**pkgmgr.exe/n:\<update-help\>**                                 |
...
|                         启动服务                          |                                                                                                                                                                                 **sc start \<service name\>** 或**net start \<service name\>**                                                                                                                                                                                  |
|                          停止服务                          |                                                                                                                                                                                  **sc 停止\<服务名称\>** 或**net stop \<服务名称\>**                                                                                                                                                                                   |
| 检索正在运行的应用程序和相关进程的列表 |                                                                                                                                                                                                           **tasklist**                                                                                                                                                                                                           |
|                        启动任务管理器                        |                                                                                                                                                                                                           **任务管理器**                                                                                                                                                                                                            |
|    创建和管理事件跟踪会话和性能日志    | 创建计数器、跟踪、配置数据收集或 API: **logman 个** <br>查询数据收集器属性: **logman 查询** <br>开始或停止数据收集: **\|logman 开始停止** <br>删除收集器: **logman 删除** <br> 更新收集器的属性: **logman 更新** <br>从 xml 文件导入数据收集器集或将其导出到 xml 文件: **logman 导入\|导出** |
...
|----|-------|
|列出事件日志|**wevtutil el**| 
|查询指定日志中的事件|**wevtutil qe/f: 文本\<日志名称\>**| 
...
|任务|Command| 
|----|-------|
|添加新硬件设备的驱动程序|将驱动程序复制到位于% homedrive%\\\<驱动程序文件夹\>的文件夹。 运行**pnputil-i-a% homedrive%\\\<驱动程序\>文件夹\>\\\<驱动程序 .inf**|
