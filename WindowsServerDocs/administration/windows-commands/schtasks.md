...
##### <a name="u-domainuser"></a>/u [@no__t > \] @ no__t-2
...
##### <a name="ru-domainuser--system"></a>/ru {[\<Domain > \] @ no__t-2 |主板
...
|每月|第一个、第二个、第三个、第四个、最后一个|与 **/d**@no__t-timeraised>now-1day > 参数一起使用，以便在特定的周和天运行任务。 例如，在月份的第三个星期三。|
...
指定任务开始的时间（每次启动时） \<HH： MM > 24 小时格式。 默认值为本地计算机上的当前时间。 **/St**参数对于分钟、每小时、每天、每周、每月和计划有效。 需要该计划。
...
有关示例，请参阅：
-   "若要计划在非工作时间运行每100分钟的任务"，请在中**计划运行每个**\<N >**分钟**的任务部分。
...
有关示例，请参阅：
-   "若要计划每隔3小时运行一次的任务，请在中**计划运行每个**\<N >**小时**的任务部分。
...
| \<DD >/<MM> @ NO__T-2 @ NO__T |       用于第一天的格式，如**保加利亚语**和**荷兰语（荷兰）** 。        |
| \<YYYY >/<MM> @ NO__T-2<DD> |          用于年份优先格式，如**瑞典语**和**法语（加拿大）** 。          |
...
每周计划还具有一个可选的 **/d**参数，用于计划任务在一周的指定天或所有天（ *）上运行。默认值为 "周一（星期一）"。每天（* ）选项相当于计划每日任务。
...
在此计划类型中， **/sc 月度**参数、 **/mo** （修饰符）参数和 **/d** （day）参数是必需的。 **/Mo** （修饰符）参数指定运行任务的周。 **/D**参数指定一周中的第几天。 （对于此计划类型，只能指定一周中的某一天。）此计划还具有一个可选 **/m** （month）参数，该参数允许你计划特定月份或每个月的任务（\*）。 **/M**参数的默认值是每月（\*）。
...
|     /s \<Computer >      |                                                                                                                                                                                                                                                                               指定远程计算机的名称或 IP 地址（带有或不带反斜杠）。 默认值为本地计算机。                                                                                                                                                                                                                                                                               |
|  /u [@no__t > \] @ no__t-2  |                                                                                                                                                                 用指定用户帐户的权限运行此命令。 默认值是本地计算机当前用户的权限。 指定的用户帐户必须是远程计算机上 Administrators 组的成员。 **/U**和 **/p**参数仅对在远程计算机上更改任务有效（ **/s**）。                                                                                                                                                                  |
|     /p \<密码 >      |                                                                                                                                                                                              指定在 **/u**参数中指定的用户帐户的密码。 如果使用 **/u**参数，但省略 **/p**参数或 password 参数，则**schtasks**会提示输入密码。</br>**/U**和 **/p**参数仅在使用 **/s**时有效。                                                                                                                                                                                               |
| /ru {[\<Domain > \] @ no__t-2 |                                                                                                                                                                                                                                                                                                                                       主板                                                                                                                                                                                                                                                                                                                                       |
...
|     /du \<Duration >     |                                                                                                                                                                                                                                                                                                     指定关闭 \<EndTime > 或 <Duration> （如果已指定）的任务。                                                                                                                                                                                                                                                                                                      |
...
|    /tn \<TaskName >    |                                                                                                                                                       必需。 标识任务。                                                                                                                                                        |
|    /s \<Computer >     |                                                                                                           指定远程计算机的名称或 IP 地址（带有或不带反斜杠）。 默认值为本地计算机。                                                                                                           |
| /u [@no__t > \] @ no__t-2 | 用指定用户帐户的权限运行此命令。 默认情况下，使用本地计算机当前用户的权限运行命令。</br>指定的用户帐户必须是远程计算机上 Administrators 组的成员。 **/U**和 **/p**参数仅在使用 **/s**时有效。 |
|    /p \<密码 >     |                          指定在 **/u**参数中指定的用户帐户的密码。 如果使用 **/u**参数，但省略 **/p**参数或 password 参数，则**schtasks**会提示输入密码。</br>**/U**和 **/p**参数仅在使用 **/s**时有效。                           |
|          /?           |                                                                                                                                                    在命令提示符下显示帮助。                                                                                                                                                     |
...
-   若要远程运行任务，则必须在远程计算机上计划任务。 运行该任务时，任务仅在远程计算机上运行。 若要验证某一任务是否正在远程计算机上运行，请使用任务管理器或任务计划程序事务日志，\<Systemroot > \SchedLgU.txt。
...
|    /s \<Computer >     |                                                                                                                        指定远程计算机的名称或 IP 地址。 默认值为本地计算机。                                                                                                                        |
| /u [@no__t > \] @ no__t-2 | 用指定用户帐户的权限运行此命令。 默认情况下，使用本地计算机当前用户的权限运行命令。 指定的用户帐户必须是远程计算机上 Administrators 组的成员。 **/U**和 **/p**参数仅在使用 **/s**时有效。 |
|    /p \<密码 >     |                        指定在 **/u**参数中指定的用户帐户的密码。 如果使用 **/u**参数，但省略 **/p**参数或 password 参数，则**schtasks**会提示输入密码。</br>**/U**和 **/p**参数仅在使用 **/s**时有效。                         |
|          /?           |                                                                                                                                                             显示帮助。                                                                                                                                                              |
...
以下命令将从本地计算机的计划中删除所有任务，包括其他用户计划的任务。 它使用 **/tn \\** * 参数来表示计算机上的所有任务，使用 **/f**参数禁止显示确认消息。
```
...
该命令使用 **/s**参数来识别远程计算机 Reskit16，使用 **/fo**参数指定格式，使用 **/nh**参数来禁止显示列标题。 **@No__t-1** append 符号会将输出重定向到本地计算机 Svr01 上的 p0102 任务日志。 由于此命令在远程计算机上运行，因此本地计算机的路径必须是完全限定的。
```
