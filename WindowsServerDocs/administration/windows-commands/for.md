...
- 使用 batch 的参数
  ...
  以下属性应用于**为**命令：  
  - **有关**命令替换 **%** <em>变量</em>或 **%%** <em>变量</em>与之前指定的命令处理的所有文件在指定集中每个文本字符串。
...
- 指定文件的组
  ...
  *设置*参数可以代表文件的单个组或多个文件组。 可以使用通配符 ( **&#42;** 并 **？** ) 指定的文件设置。 以下是有效的文件集：  
  ...
  (ar??1991.* ap??1991.*)
  ```  
  当你使用**有关**命令中的第一个值*设置*会替换 **%** <em>变量</em>或 **%%** <em>变量</em>，然后再指定的命令处理此值。 这将继续，直到与相对应的所有文件 （或文件组）*设置*值进行处理。
- 使用**中**并**执行**关键字
