...
以下命令使用注册表项 HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows NT\Currentverion @ no__t 创建名为 cfg_log 的配置数据收集器。  
```  
logman create cfg cfg_log -reg "HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows NT\Currentverion\\"  
```  
以下命令将创建一个名为 cfg_log 的配置数据收集器，用于记录数据库列 MSNdis_Vendordriverversion 中 root\wmi 的所有 WMI 对象。  
```  
