...
|键|定义|ReplTest1|示例|
|---|----------|-----|-------|
|Subject|多个应用程序依赖于证书的使用者信息。 因此，建议指定此密钥的值。 如果使用者未在此处设置，则建议使用者名称是作为使用者可选名称证书扩展的一部分。|相对可分辨名称字符串值|Subject = "CN=computer1.contoso.com" Subject="CN=John Smith,CN=Users,DC=Contoso,DC=com"|
