...
3.  在收集器计算机上，运行以下命令之一：  
    ...
    -   在 Windows PowerShell 提示符下： `Set-Item -Force WSMan:\localhost\Client\TrustedHosts "<target1>,<target2>,..."`，后跟 `Set-Item -Force WSMan:\localhost\Client\AllowUnencrypted true` （其中 \<target1 > 等）是目标计算机的名称或 IP 地址。  
...
5.  在目标计算机上，打开一个提升的 Windows PowerShell 提示符并运行以下命令：  
    ...
    此处 < target_name > 是目标计算机的名称，\<ip > 是收集器计算机的 IP 地址。 \<port > 是收集器将在其中运行的端口号。 密钥 <a.b.c.d> 是通信所需的加密密钥，它包括由点分隔的四个字母数字字符串。 收集器计算机上使用此相同的密钥。 如果你未输入密钥，则系统会生成一个随机密钥；你需要将此密钥用于收集器计算机，因此请记下此密钥。  
...
2.  在打开的菜单中，单击 "**属性**"，然后单击 "**详细信息**" 选项卡。展开 "**属性**" 字段中的菜单，滚动到 "查找**位置信息**" （列表可能不是按字母顺序），然后单击它。 该值将是一个字符串，其形式为**PCI Bus X、设备 Y 和函数 Z**。记下 X.x.x.x;以下命令是你需要用于以下命令的总线参数。  
...
-   Hostip @no__t = 收集器的 > 地址  
  
-   Port = 您为收集器指定 @no__t 0port 的数字 >  
...
3.  将此示例配置复制到记事本窗口中：  
    ...
    > [!NOTE]  
    > 根节点 \<collector >。 其属性指定配置文件语法的版本和状态日志文件的名称。  
    >   
    > @No__t-0common > 元素将多个目标组合在一起，从而为它们指定常见的配置元素，非常类似于用户组可用于指定多个用户的通用权限。  
    >   
    > @No__t-0collectorport > 元素定义收集器侦听传入数据的 UDP 端口号。 这是 Bcdedit 目标配置步骤中指定的相同端口。 收集器仅支持一个端口，并且所有目标都必须连接到同一个端口。  
    >   
    > @No__t-0forwarder > 元素指定如何转发从目标计算机接收的 ETW 事件。 只有一类转发器会将事件写入 ETL 文件。 参数指定文件名模式、圈中每个文件的大小限值以及每台计算机的圈大小。 设置“toxml”指定接收 ETW 事件时将按二进制形式写入，不会转换为 XML。 有关决定是否将事件赋予 XML 的信息，请参阅“XML 事件转换”部分。 文件名模式包含以下替换：用 {computer} 替换计算机名，用 {#3} 替换圈中的文件索引。  
    >   
    > 在此示例文件中，两个目标计算机是用 \<target > 元素定义的。 每个定义都指定了包含 @no__t 0ipv4 > 的 IP 地址，但你也可以使用 MAC 地址（例如，< mac 值 = "11：22：33：44：55： 66" \/ > 或 < MAC 值 = "11-22-33-55-66" @no__t）或 SMBIOS GUID （例如 > GUID值 = "{269076F9-4B77-46E1-B03B-CA5003775B88}" @no__t >）以标识目标计算机。 另外，请记录加密密钥（与在目标计算机上使用 Bcdedit 指定或生成的密钥相同）以及计算机名称。  
  
4.  在配置文件中输入每台目标计算机的详细信息作为单独的 @no__t 0target > 元素，并保存 Newconfig.json 并关闭记事本。  
...
在配置文件的 \<collector > 元素中设置最小日志级别。 例如： < 收集器 configVersionMajor = "1" minlog @ no__t-0 "verbose" >。  
  
详细级别记录了处理每个数据包时收到的数据包记录。 调试级别还会添加更多处理详细信息，并转储所有收到的 ETW 数据包的内容。  
  
在调试级别，你可以将日志写入文件，而非尝试在常用的日志记录系统中查看日志。 为此，请在配置文件的 \<collector > 元素中添加一个附加元素：  
...
2. 若要查看收集器的扩展诊断，请将此项添加到配置文件的 \<collector > 元素：  
   \<collector .。。minlog = "verbose" >  
   这将启用有关每个收到的数据包的消息。  
3. 检查是否确实收到了任何数据包。 （可选）你可能希望以详细模式将日志直接写入文件，而不是通过 ETW 写入文件。 为此，请将以下内容添加到配置文件 @no__t 的 0collector > 元素：  
...
|收集器||未创建 ETL 文件。|`Get-SbecForwarding` 显示目标已连接，没有错误，但未创建 ETL 文件。|目标计算机可能尚未发送任何数据；仅在收到数据时才会创建 ETL 文件。|  
