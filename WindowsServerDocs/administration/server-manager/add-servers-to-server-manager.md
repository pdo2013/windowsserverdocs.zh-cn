...
## <a name="BKMK_creds"></a>使用“管理方式”命令提供凭据
...
请按照本主题中的过程添加要管理的服务器，在使用**管理方式**命令指定可能需要用于管理所添加服务器的备用凭据之前，服务器可能会出现以下可管理性状态错误：
...
3.  执行下列操作之一。
    ...
    -   在 **Active Directory** 选项卡中，选择位于当前域中的服务器。 选择时按下 **Ctrl** 以选择多台服务器。 单击右箭头按钮以将选定服务器移到**选定**列表。
...
5.  如果运行服务器管理器的计算机和目标工作组服务器位于同一工作组，请跳到最后一步。 如果两台计算机不位于同一工作组，请在**服务器**磁贴中右键单击目标工作组服务器，然后单击**管理方式**。
