...
- 步骤 2：打印服务器上安装混合云打印包
- 步骤 3:安装 Azure 应用程序代理 (AAP) 使用 Kerberos 约束委派 (KCD)
- 步骤 4：配置所需的 MDM 策略
- 步骤 5：发布的共享的打印机
...
### <a name="step-3---install-azure-application-proxy-aap-with-kerberos-constrained-delegation-kcd"></a>步骤 3-Azure 应用程序代理 (AAP) 安装了 Kerberos 约束委派 (KCD)
...
3. 在 Active Directory 计算机，请转到**工具-> 用户和计算机**
    - 选择运行应用程序代理连接器的计算机
    - 右键单击并选择**属性-> 委派**选项卡
    - 选择**信任此计算机来委派指定的服务。**
    - 选择**使用任何身份验证协议。**
    - 下**此帐户可以为其提供委派的凭据的服务**
        - 添加运行服务 （MopriaDiscoveryService 和 MicrosoftEnterpriseCloudPrint 服务） 的计算机的 SPN 标识值
            - 对于 SPN，输入的 SPN，计算机本身即"主机 /\<MachineName\>。\<域\>"<br>
...
4. 返回到 AAD 租户管理门户并添加应用程序代理
   ...
   - 创建后，请返回到**企业应用程序** -> **的所有应用程序**，选择刚创建的新应用程序
   - 转到**单一登录**，请确保"单一登录模式"设置为"集成 Windows 身份验证"
   - 将"内部应用程序 SPN"设置为上述步骤 3.3 中, 指定的 SPN
   - 请确保"委派的登录标识"设置为"用户主体名称"

5. 对企业云打印服务更高版本，重复第 4，并提供你的企业云打印服务的内部 URL
6. 返回到 Azure AD 租户管理门户并转到**应用注册**和选择的本机客户端应用->"设置"
    - 选择**所需的权限**
        - 添加 2 个新代理的应用程序只需创建
        - 对于这些 2 应用程序授予委派权限
        - 添加这两个代理应用程序后，单击"授予权限"。  选择"是"当系统提示你批准请求。

7. 启用 Windows 身份验证在 IIS 中为 Mopria 云服务和企业云打印服务计算机
    - 请确保已安装 Windows 身份验证功能：
        ...
        4. 选择**基于角色或基于功能的安装**
        5. 选择服务器
        6. 在 Web 服务器 (IIS)-> Web 服务器安全-> 中，选择**Windows 身份验证**
        7. 单击下一步直到完成安装
    - 启用 Windows 身份验证在 IIS 中：
        1. 打开 Internet Information Services (IIS) 管理器
        2. 为每个服务/站点：
            1.  选择的服务/站点
            2.  双击**身份验证**
            3.  单击**Windows 身份验证**然后单击**启用**下**操作**
...
- 查找企业云打印策略组和配置策略遵循以下指导原则：
  ...
  - MopriaDiscoveryResourceId = Mopria 发现服务 Azure 应用程序代理的步骤 3.4 中创建的外部 URL (必须完全相同包括尾随 /)
  - CloudPrintResourceId = 企业云打印服务 Azure 应用程序代理的步骤 3.5 中创建的外部 URL (必须完全相同包括尾随 /)
...
- OMA-URI
    ...
    - `MopriaDiscoveryResourceId = ./Vendor/MSFT/Policy/Config/EnterpriseCloudPrint/MopriaDiscoveryResourceId`
        - 值 = Mopria 发现服务 Azure 应用程序代理的步骤 3.4 中创建的外部 URL (必须完全相同包括尾随 /)
    - `CloudPrintResourceId = ./Vendor/MSFT/Policy/Config/EnterpriseCloudPrint/CloudPrintResourceId`
        - 值 = 企业云打印服务 Azure 应用程序代理的步骤 3.5 中创建的外部 URL (必须完全相同包括尾随 /)
...
5. 在 Windows 10 Fall Creator Update 计算机中，打开提升的 Windows PowerShell 命令提示符
   1. 运行以下命令
      ...
      - Publish-CloudPrinter
        ...
        - DiscoveryEndpoint = Mopria 发现服务 Azure 应用程序代理的步骤 3.4 中创建的外部 URL
        - PrintServerEndpoint = 企业云打印服务 Azure 应用程序代理的步骤 3.5 中创建的外部 URL
...
- 打开 web 浏览器并转到 https://&lt;services 机终结点 &gt; /mcs/服务 (发现终结点的外部 URL)
