...
2. 输入以下信息：
   ...
   - **用户帐户**-添加用于访问远程 PC 的用户帐户。
     - 对于加入 Active Directory (AD) 的计算机或本地帐户，使用以下格式之一： *user_name*， *domain\user_name*，或<em>user_name@domain.com</em>。
     - 有关 Azure Active Directory (AAD) 加入的计算机，使用以下格式之一：*AzureAD\user_name*或<em>AzureAD\user_name@domain.com</em>。
     - 此外可以选择是否需要密码。
