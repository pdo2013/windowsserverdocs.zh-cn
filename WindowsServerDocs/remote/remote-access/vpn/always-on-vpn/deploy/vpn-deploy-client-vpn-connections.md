...
此步骤中，了解 ProfileXML 选项和架构，并配置 Windows 10 客户端计算机与该基础结构与 VPN 连接进行通信。 
...
## <a name="bkmk_profile"></a>手动创建的模板连接配置文件

在此步骤中，使用受保护的可扩展身份验证协议\(PEAP\)以确保客户端和服务器之间的通信安全。 与不同的是简单的用户名和密码，此连接需要在要工作的 VPN 配置文件中的唯一 EAPConfiguration 部分。 
...
6.  记录的值**颁发给证书**并**颁发者**。<p>即将推出的 VPN 模板配置中使用这些值。 例如，如果服务器的 FQDN 是 nps01.corp.contoso.com 和主机名是 NPS01，证书名称基于服务器-例如，nps01.corp.contoso.com 的 FQDN 或 DNS 名称。
...
### <a name="configure-the-template-vpn-profile-on-a-domain-joined-client-computer"></a>域上配置 VPN 配置文件的模板\-加入的客户端计算机

现在，已加入域的客户端计算机上配置 VPN 配置文件的模板所需的信息。 用用户帐户的类型\(也就是说，标准用户或管理员\)对于此过程的一部分并不重要。 
...
6.  在服务器名称或地址，键入**外部**VPN 服务器的 FQDN\(等 **vpn.contoso.com**\)。
...
12. 在设置中，通过单击测试 VPN**模板**，然后单击**Connect**。
...
完成本部分之前, 请确保已创建并测试模板 VPN 连接的部分[手动创建的模板连接配置文件](#bkmk_profile)描述。 测试 VPN 连接，才可确保配置文件包含到 VPN 连接所需的所有信息。
...
1. 登录到包含该模板具有相同的用户的 VPN 配置文件的已加入域的客户端计算机帐户，部分"手动创建的模板连接配置文件"所述。

2. 将列表 1 中粘贴到 Windows PowerShell 集成脚本环境\(ISE\)，并自定义在注释中所述的参数。 这些是 $Template、 $ProfileName、 $Servers、 $DnsSuffix、 $DomainName、 $TrustedNetwork 和 $DNSServers。 在注释是每个设置的完整说明。
...
>[!NOTE]
>若要查看完整的示例脚本，请参阅明[MakeProfile.ps1 完整脚本](#bkmk_fullscript)。
...
## <a name="vpn-deploy-client-sccm"></a>使用 System Center Configuration Manager 中配置 VPN 客户端

在 System Center Configuration Manager 中，可以通过使用 ProfileXML CSP 节点中，就像在 Windows PowerShell 中一样来部署 VPN 配置文件。 在这里，使用的部分中创建的 VPN_Profile.ps1 Windows PowerShell 脚本[创建 ProfileXML 配置文件](#bkmk_ProfileXML)。
...
创建用户组能够接收 VPN 配置文件后，可以创建包和程序部署的部分中创建的 Windows PowerShell 配置脚本[创建 ProfileXML 配置文件](#bkmk_ProfileXML)。
...
4.  在包页面上，完成以下步骤：
    ...
    c. 在设置源文件夹对话框中，单击**浏览**，选择包含 VPN_Profile.ps1，文件共享，然后单击**确定**。<p>请确保你选择的网络路径，而不是本地路径。 换而言之，路径应类似于 *\\fileserver\\vpnscript*，而不*c:\\vpnscript*。
...
若要使用 Intune 来部署 Windows 10 的远程访问始终在 VPN 配置文件，可以配置 ProfileXML CSP 节点使用的部分中创建的 VPN 配置文件[创建 ProfileXML 配置文件](#bkmk_ProfileXML)，也可以使用基 EAP下面提供的 XML 示例。
...
5.   下**平台**，选择**Windows 10 或更高版本**，然后选择**VPN**从配置文件类型下拉列表。
...
8.  替换 **\<TrustedRootCA > 5a 89 fe cb 5b 49 a7 0b 1a 52 63 b7 35 ee d7 1c c2 68 是 4b <\/TrustedRootCA >** 在示例中的本地根证书颁发机构的证书指纹在这两个位置。
...
10. 替换 **\<ServerNames > NPS.contoso.com\</ServerNames >** 中的身份验证发生在已加入域的 NPS fqdn 的 XML 示例。 

11. 复制修改后的 XML 字符串并粘贴到**EAP Xml**在基础 VPN 选项卡下的框，然后单击**确定**。<p>在 Intune 中创建使用 EAP Always On VPN 设备配置策略。
