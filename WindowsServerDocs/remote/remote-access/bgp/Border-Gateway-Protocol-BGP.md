...
当在多租户模式下的 Windows Server 2016 远程访问服务 \(RAS @ no__t Gateway 上配置时，边界网关协议（BGP）使你能够管理租户的 VM 网络与其远程网络流量之间的网络流量路由节点. 你还可以将 BGP 用于单租户 RAS 网关部署，并将远程访问部署为局域网 \(LAN @ no__t）。  
  
BGP 可减少对在路由器上进行手动路由配置的需要，因为它是一种动态路由协议，可自动了解使用站点到站点 VPN 连接进行连接的站点之间的路由。  
  
若要使用 BGP 路由，你必须在计算机或虚拟机上安装远程访问服务器角色 **\(RAS @ no__t**和/或**路由**角色服务，\(VM @ no__t-你使用的系统类型取决于是否具有多租户部署：  
  
-   对于多租户部署，建议在一个或多个 Vm 上安装 RAS 网关。 使用多个 Vm 可提供高可用性。 RAS 网关能够处理来自多个租户的多个连接，由 Hyper-v 主机以及实际配置为网关的 VM 组成。 此网关使用站点到站点 VPN 连接配置为多租户 BGP 路由器，用于交换租户和云服务提供商 \(CSP @ no__t 子网路由。  
