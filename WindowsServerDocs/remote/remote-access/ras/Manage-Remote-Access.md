...
-   **轻松访问**：运行 Windows 8 或 Windows 7 的托管客户端计算机可以配置为 DirectAccess 客户端计算机。 这些客户端只要连接到 Internet，就可以随时通过 DirectAccess 访问内部网络资源，而无需登录 VPN 连接。 未运行这些操作系统之一的客户端计算机可以通过 VPN 连接到内部网络。 DirectAccess 和 VPN 由同一控制台管理并使用相同的向导集。  
