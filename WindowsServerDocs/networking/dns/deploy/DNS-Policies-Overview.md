...
- **客户端子网：** 客户端子网对象表示从其查询提交到 DNS 服务器的 IPv4 或 IPv6 子网。 您可以创建要更高版本定义策略，以应用根据请求来自的子网的子网。 例如，在裂脑 DNS 情况，如名称解析的请求*www.microsoft.com*可以找到相应的内部 IP 地址向客户端从内部子网，并向外部客户端不同的 IP 地址子网。
...
|**FQDN**|在查询中，记录的因为它使用通配符的 FQDN|-   **EQ、 www.contoso.com** -解析总数 rue 仅当查询在尝试解析*www.contoso.com* FQDN<br />-   **EQ、\*。 contoso.com\*。 woodgrove.com** -解析为 true，如果查询的任何记录以结尾*contoso.com***或***woodgrove.com*|  
