...
|MaxAllowedPhaseOffset|全部|项指定 W32Time 尝试使用时钟速率调整计算机时钟的最大偏移 （以秒为单位）。 当偏移量超出该速率时，W32Time 将直接设置的计算机时钟。 域成员的默认值为 300。 独立客户端和服务器的默认值为 1。  [有关详细信息，请参阅下面](#MaxAllowedPhaseOffset)。|
...
SystemclockRate 是时钟的在系统上的速率。 例如使用 156000 秒，SystemclockRate 会为 = 0.0156000 * 1000 \* 10000 = 156000 时钟计时周期数。  
...
下表列出了与 Windows 时间服务，并关联与每个设置的预设的值相关联的全局组策略设置。 有关每个设置的详细信息，请参阅中的相应注册表项"[Windows 时间服务注册表项](#w2k3tr_times_tools_uhlp)"前面的这一主题。 以下设置包含在一个单一的 GPO，称为**全局配置设置**。  
...
下表列出的可用设置**配置 Windows NTP 客户端**GPO 和 Windows 时间服务与关联的预设的值。 有关每个设置的详细信息，请参阅中的相应注册表项"[Windows 时间服务注册表项](#w2k3tr_times_tools_uhlp)"前面的这一主题。  
