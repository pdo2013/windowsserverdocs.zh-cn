...
7.  中的 HYPER-V 管理器下**虚拟机**，右键单击你想要配置，然后单击 VM**设置**。  
...
13. 若要在中添加的 HYPER-V 管理器中的第二个的网络适配器**虚拟机**，右键单击同一个 VM，然后单击**设置**。  
...
2. 在中**属性**窗格中，在第一列中，找到**NIC 组合**，然后单击**禁用**链接。  
...
4. 单击**任务**，然后单击**将添加到新的团队**。  

   **新的团队**对话框将打开并显示网络适配器和团队成员。
...
7. 如果你想要配置主要组接口名称或将 VLAN 分配到 NIC 组，请单击右侧的链接**主要组接口**。  

    **新的团队界面**对话框随即打开。

    ![新组接口](../../media/Create-a-New-NIC-Team/nict_newteaminterface.jpg)
