...
## <a name="FAQ1"></a> Nano Server 是否支持存储副本？  
是的。  

> [!NOTE]
> 在安装期间必须使用 **Storage** Nano Server 包。 有关部署 Nano Server 的详细信息，请参阅 [Nano Server 入门](https://technet.microsoft.com/library/mt126167.aspx)。  

使用 PowerShell 远程处理在 Nano Server 上安装存储副本，具体操作如下所示：  

1. 将 Nano Server 添加到客户端信任列表。  
   > [!NOTE]
   > 仅当计算机不是 Active Directory 域服务林的成员或在不受信任的林中时，才有必要执行此步骤。 这会将 NTLM 支持添加到 PSSession 远程处理，出于安全原因该支持默认处于禁用状态。 有关详细信息，请参阅 [PowerShell 远程处理安全注意事项](https://msdn.microsoft.com/powershell/scriptiwinrmsecurity)。  

   ```  
       Set-Item WSMan:\localhost\Client\TrustedHosts "<computer name of Nano Server>"  
   ```  
2.  若要安装存储副本功能，请从管理计算机运行以下 cmdlet：  

    ```  
    Install-windowsfeature -Name storage-replica,RSAT-Storage-Replica -ComputerName <nano server> -Restart -IncludeManagementTools  
    ```  

    在 Windows Server 2016 中结合使用 `Test-SRTopology` cmdlet 和 Nano Server 需要通过 CredSSP 进行远程脚本调用。 与其他存储副本 cmdlet 不同，`Test-SRTopology` 要求在源服务器上本地运行。   
在 Nano Server 上（通过远程 PSSession 进行操作）：  

    >[!NOTE]
    >CREDSSP 用于在 `Test-SRTopology` cmdlet 中提供 Kerberos 双跃点支持，但其他存储副本 cmdlet 中不需要此 CREDSSP，它们可以自动处理分布式系统凭据。 通常情况下，建议不要使用 CREDSSP。 有关 CREDSSP 的替代方法，请查看以下 Microsoft 博客文章："PowerShell Remoting Kerberos Double Hop Solved Securely" - https://blogs.technet.microsoft.com/ashleymcglone/2016/08/30/powershell-remoting-kerberos-double-hop-solved-securely/ 
    ...
    在管理计算机上：  
       ...
       然后，将结果复制到管理计算机或共享路径。 因为 Nano 缺少必要的图形库，可以使用 Test-SRTopology 来处理结果，并提供带有图表的报表文件。 例如：  

        Test-SRTopology -GenerateReport -DataPath \\sr-srv05\c$\temp  


## <a name="FAQ2"></a> 如何在初始同步期间查看复制进度？  
...
## <a name="FAQ3"></a>能否指定特定网络接口用于进行复制？  
...
## <a name="FAQ4"></a> 能否配置一对多复制或可传递（A 到 B 到 C）复制？  
...
## <a name="FAQ5"></a> 能否增大或收缩由存储副本复制的已复制卷？  
可以增大（扩展）卷，但不能收缩它们。 默认情况下，存储副本会阻止管理员扩展复制卷；请在调整大小之前对源组使用 `Set-SRGroup -AllowVolumeResize $TRUE` 选项。 例如：

1. 用于源计算机： `Set-SRGroup -Name YourRG -AllowVolumeResize $TRUE`
2. 使用你喜欢的任何技术增大卷
3. 用于源计算机： `Set-SRGroup -Name YourRG -AllowVolumeResize $FALSE` 

## <a name="FAQ6"></a>能否使目标卷联机以便进行只读访问？  
...
复制的卷 D: 现在可以在 SRV2 上访问。 你可以正常对其进行读写、从其中复制文件或运行保存在其他地方妥善保管的联机备份。
...
## <a name="FAQ7"></a>能否在拉伸群集中配置横向扩展文件服务器 (SOFS)？  
虽然从技术上来说，这是可以实现的，但建议不要在 Windows Server 2016 中进行此配置，因为联系 SOFS 的计算节点中缺少站点感知。 如果使用的是校园网，其延迟通常是亚毫秒级的，此配置通常会正常运行，而不会出现问题。   

在两个群集间进行复制时，如果配置群集到群集复制，存储副本会完全支持横向扩展文件服务器，包括可以使用存储空间直通。  

## <a name="FAQ8"></a>能否在具有存储副本的拉伸群集中配置存储空间直通？  
...
还可以使用植入的数据卷，有时通过确保目标卷由还原的备份、旧快照、以前复制和已复制的文件等具有主卷的一些数据子集，然后使用故障转移群集管理器中的植入选项或使用 `New-SRPartnership` 来减少带宽的使用。 如果该卷大部分为空，则使用植入的同步会减少时间和带宽的使用量。

## <a name="FAQ13"></a> 能否委托用户管理复制？  
...
## <a name="FAQ13"></a> 针对已复制卷，我的备份和还原选项是什么？
...
存储副本依赖 SMB 和 WSMAN 进行其复制和管理。 这意味着必需使用以下端口：
   ...
   445（SMB - 复制传输协议）5445（iWARP SMB - 仅在使用 iWARP RDMA 网络时需要）5895（WSManHTTP - WMI/CIM/PowerShell 的管理协议）

注意：Test-SRTopology cmdlet 需要 ICMPv4/ICMPv6，但不用于复制或管理。

## <a name="FAQ15"></a>日志卷最佳做法是什么？
...
SR 性能计数器可告知日志的改动率，从而帮助做出一些判断。 Test-SRTopology cmdlet 也可执行此操作。 一般情况下，可查看现有工作负荷的写入 IO 以确定每分钟、每小时或每天可能流经日志的 IO 量。
...
## <a name="FAQ16"></a> 为何选择拉伸群集、群集到群集或服务器到服务器拓扑？  
...
群集到群集拓扑使用两个独立群集，非常适用于需要手动故障转移的管理员，尤其是当预配第二个站点以进行灾难恢复以及无需每日使用的时候。 业务流程为手动操作。 与拉伸群集不同，存储空间直通可用于此配置。 使用至少四个节点进行运行。 
...
## <a name="FAQ17"></a> 如何报告存储副本或本指南中的问题？  
