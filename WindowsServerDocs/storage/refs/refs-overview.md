...
- **[镜像加速奇偶校验](./mirror-accelerated-parity.md)** - 镜像加速奇偶校验既可以提供高性能，也可为你的数据提供高效的容量存储。 
    ...
    - 在配置了这些层后，ReFS 就会使用它们为热数据提供快速存储，以及为冷数据提供节省空间的存储：
        ...
        - 如果在使用 [混合部署](../storage-spaces/choosing-drives.md)（混用闪存和 HDD 驱动器），则 [存储空间直通中的缓存](../storage-spaces/understand-the-cache.md) 将有助于加快读取速度，从而降低虚拟化工作负载的数据碎片特性的影响。 否则，如果使用全闪存部署，则读取还将在性能层中发生。 

>[!NOTE]
>对于服务器部署，镜像加速奇偶校验仅在[存储空间直通](../storage-spaces/storage-spaces-direct-overview.md)上受支持。
...
- 借助完整性流、联机修复和备用数据副本，ReFS 和存储空间直通能够一起检测到和更正元数据和数据内的损坏。 
- ReFS 提供扩展和支持大量数据集的功能。 

### <a name="storage-spaces-with-sas-drive-enclosures"></a>具有 SAS 驱动器机箱的存储空间 ###
在具有共享 SAS 机箱的存储空间上部署 ReFS 适合于托管存档数据和存储用户文档：
- 借助完整性流、联机修复和备用数据副本，ReFS 和存储空间能够一起检测到和更正元数据和数据内的损坏。 
- 存储空间部署还可以利用块克隆和 ReFS 中提供的可扩展性。

### <a name="basic-disks"></a>基本磁盘 ###
在基本磁盘上部署 ReFS 最适合于实现自身软件复原和可用性解决方案的应用程序。 
...
| 最大文件大小 | 18 EB（艾字节）  | 18 EB（艾字节）                |
| 最大卷大小 | 4.7 ZB（泽字节）                           | 256 TB                |
