...
2. 在 "**输入凭据**" 页上，键入在要从中进行迁移的服务器上工作的管理员凭据，然后选择 "**下一步**"。

   > [!NOTE]
   > 如果选择从 Linux Samba 服务器迁移，则还需要执行额外的步骤来提供 SSH 密码或私钥

3. 选择 "**添加设备**"，键入源服务器名称，然后选择 **"确定"** 。 <br>对于要清点的任何其他服务器，请重复此步骤。

   > [!NOTE]
   > 如果选择从故障转移群集进行迁移，请提供群集文件服务器资源的名称。
...
3. 在 "**配置**切换" 页上，指定要对每个源设备设置采用的网络适配器。 这会在转换过程中将 IP 地址从源移动到目标。 您可以选择跳过所有网络迁移或某些接口。 如果要在服务器上切削，则必须始终为源接口指定 DHCP 或新的静态 IP 地址。
