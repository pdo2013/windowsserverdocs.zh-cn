...
> 适用于：Windows Server 2016、Windows 10（1607 版）
...
- [在 Windows Server 2016 中将非易失性内存 (NVDIMM N) 用作按字节寻址存储](https://channel9.msdn.com/Events/Build/2016/P470)
- [加快 SQL Server 2016 性能与 Windows Server 2016 中的永久内存](https://channel9.msdn.com/Shows/Data-Exposed/SQL-Server-2016-and-Windows-Server-2016-SCM--FAST)
...
|存储空间特性|存储空间仍可运行（前提是只有 1 个 NVDIMM N 受影响）。<br>NVDIMM-N PhysicalDisk 对象的“运行状况状态”显示为“警告”和“通用物理磁盘”|
|详细信息|PhysicalDisk 对象的 OperationalStatus 字段。 <br>EventLog – Microsoft-Windows-ScmDisk0101/Operational|
...
|存储空间特性|如果只有一个 NVDIMM 受到影响，那么存储空间仍可以运行。<br>NVDIMM-N 物理磁盘对象的运行状况状态将显示为“不正常”，无法用于存储空间。|
|详细信息|PhysicalDisk 对象的 OperationalStatus 字段。<br>EventLog – Microsoft-Windows-ScmDisk0101/Operational|
