...
3. **NVMe + SSD + HDD。** 如果有所有三种类型的驱动器，NVMe 驱动器会对 SSD 和 HDD 进行缓存。 有吸引力的地方在于，你可以在 SSD 上创建卷以及在 HDD 上创建卷，它们并行位于同一个群集中，并且全都由 NVMe 分配。 前者就像是在“全闪存”部署环境中，而后者就像是在上文所述的“混合”部署环境中。 这在概念上类似具有两个池，主要采用独立的容量管理以及失败和维修周期等。
   ...
   >[!IMPORTANT]
   > 我们建议使用 SSD 层，以将对性能最敏感的工作负载放置在全闪存中。
...
>[!IMPORTANT]
>仅不支持使用 Hdd 进行配置。 不建议对低耐用性 Ssd 缓存的高的耐用性 Ssd。
...
应调整缓存大小以适应你的应用程序和工作负载，即所有数据主动读取或写入在任何给定时间的工作集。 没有除此之外的缓存大小要求。 对于使用 Hdd 的部署，适当的起始位置是 10%的容量-例如，如果每个服务器具有 4 x 4 TB HDD = 16 TB 的容量，则 2 x 800 GB SSD = 1.6 TB 的每个服务器的缓存。 对于全闪存部署，尤其是与非常[高的耐用性](https://blogs.technet.microsoft.com/filecab/2017/08/11/understanding-dwpd-tbw/)Ssd，它可能会公平开始接近 5%的容量-例如，如果每个服务器都具有全天候 1.2 TB SSD = 28.8 TB 的容量，然后 750 GB NVMe x 2 = 1.5 TB 的每个服务器的缓存。 你始终可以在以后添加或移除缓存驱动器来进行调整。
