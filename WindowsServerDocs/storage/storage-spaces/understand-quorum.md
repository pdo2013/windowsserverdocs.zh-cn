...
- **群集仲裁**:此操作的过程在群集级别 （即您可以丢失节点并具有仍然能正常运行的群集）
- **池仲裁**:此操作的过程在池级别上启用存储空间直通 （即可以丢失节点和驱动器并将仍然能正常运行的池）。 存储池是专门设计用来在聚集和非群集方案中，正因如此，它们具有不同的仲裁机制。
...
- 如果你有两个节点，见证服务器是**需**。
- 有三个或四个节点，见证服务器是否**强烈建议**。
- 如果您能够访问 Internet，则使用 **[云见证服务器](../../failover-clustering/deploy-cloud-witness.md)**
...
只要仍然运行的节点已成功验证它们是否*多数*的定义*大多数*更新为之间只是时更新。 这允许群集失去一个节点，然后另一个，则另一个，等等。 这一概念*总投票数*连续失败后调整称为***动态仲裁***。  
...
- 如果有**甚至**节点，并没有见证服务器，数字*一个节点获取归零其投票*。 例如，只有这三个四个节点获取投票，因此*总投票数*是三种模型，并使用投票的两个幸存者被视为大多数。
- 如果有**奇数**节点和没有见证服务器的数目*它们都获取投票*。
- 如果有**甚至**的节点以及见证服务器，数字*见证投票*，因此总共为奇数。
- 如果有**奇数**的节点以及见证服务器，数字*见证服务器不会进行投票*。
...
上面的方案适用于不具有存储空间直通启用的常规群集。 但是，启用存储空间直通后，群集只能支持两个节点故障。 对此进行更[池仲裁部分](#pool-quorum-overview)。

### <a name="examples"></a>示例

#### <a name="two-nodes-without-a-witness"></a>不使用见证服务器的两个节点。 
一个节点的投票信息为零，因此*多数*投票决定共**1 投票**。 如果非投票节点意外出现故障，存活的 1/1 和群集仍然有效。 如果投票节点意外出现故障，存活具有 0/1 和群集出现故障。 如果投票节点正常关机，投票传输到另一个节点，群集可以幸存，但。 ***这是很重要配置见证服务器的原因。***

![不使用见证服务器的两个节点与用例中所述的仲裁](media/understand-quorum/2-node-no-witness.png)

- 可以保留一个服务器故障：**各有 50%机会**。
- 一台服务器出现故障，则另一个可以经受得住：**否**。
- 可以两个服务器从故障中恢复次：**否**。 

#### <a name="two-nodes-with-a-witness"></a>具有见证服务器的两个节点。 
这两个节点进行投票，加上投票见证服务器，因此*多数*确定共**3 投票**。 如果其中一个节点出现故障，存活有 2/3，群集可以幸存，但。
...
所有节点进行都投票，因此*多数*确定共**3 投票**。 如果任何节点出现故障，时更新是 2/3 和群集仍然有效。 群集变得不使用见证服务器的两个节点 – 此时，您在第一种情况。

![不使用见证服务器的三个节点与用例中所述的仲裁](media/understand-quorum/3-node-no-witness.png)

- 可以保留一个服务器故障：**是**。
- 一台服务器出现故障，则另一个可以经受得住：**各有 50%机会**。
- 可以两个服务器从故障中恢复次：**否**。 

#### <a name="three-nodes-with-a-witness"></a>具有见证服务器的三个节点。
所有节点进行都投票，因此见证服务器最初不会进行都投票。 *多数*确定共**3 投票**。 后一次失败，群集具有两个节点具有见证服务器-这是返回到第二种情况。 因此，现在两个节点和见证投票。

![具有见证服务器仲裁带有三个节点的用例中所述](media/understand-quorum/3-node-witness.png)

- 可以保留一个服务器故障：**是**。
- 一台服务器出现故障，则另一个可以经受得住：**是**。
- 可以两个服务器从故障中恢复次：**否**。 

#### <a name="four-nodes-without-a-witness"></a>不使用见证服务器的四个节点
一个节点的投票信息为零，因此*多数*确定共**3 投票**。 一次失败后群集变得三个节点，并且您在方案 3。

![不使用见证服务器的四个节点与用例中所述的仲裁](media/understand-quorum/4-node-no-witness.png)

- 可以保留一个服务器故障：**是**。
- 一台服务器出现故障，则另一个可以经受得住：**是**。
- 可以两个服务器从故障中恢复次：**各有 50%机会**。 

#### <a name="four-nodes-with-a-witness"></a>具有见证服务器的四个节点。
所有节点投票和见证服务器才能正常运行，因此*多数*确定共**5 个投票**。 一次失败后在方案 4。 后两个同时发生故障，则跳过第二种情况下。
...
- **[云见证](../../failover-clustering/deploy-cloud-witness.md)** -中的 Blob 存储可访问 Azure 的所有节点的群集。 它会维护维护 witness.log 文件中的聚类分析信息，但不存储群集数据库的副本。
- **文件共享见证**– 运行 Windows Server 的文件服务器配置的 SMB 文件共享。 它会维护维护 witness.log 文件中的聚类分析信息，但不存储群集数据库的副本。
- **磁盘见证**-一个小的群集的磁盘是群集可用存储组中。 该磁盘是高度可用，可以在节点之间故障转移。 它包含群集数据库的副本。  ***磁盘见证服务器不支持使用存储空间直通***。
...
每个 16 个驱动器都有一票和两个节点还具有一票 （因为它是池资源所有者）。 *多数*确定共**16 投票**。 如果三个和四个节点出现故障，则存活的子集具有 8 个驱动器和池资源所有者，这是 9/16 投票。 因此，在池中仍然有效。
...
每个 16 个驱动器都有一票和节点 2 （因为它是池资源所有者） 还具有一票。 *多数*确定共**16 投票**。 首先，驱动器 7 出现故障。 如果三个和四个节点出现故障，则存活的子集具有 7 驱动器和池资源所有者，即 8/16 投票。 因此，该池没有大多数和出现故障。
...
每 24 个驱动器都有一票并两个节点还具有一票 （因为它是池资源所有者）。 *多数*确定共**24 投票**。 如果三个和四个节点出现故障，则存活的子集具有 8 个驱动器和池资源所有者，这是 9/24 投票。 因此，该池没有大多数和出现故障。

![池仲裁 3](media/understand-quorum/pool-3.png)

- 可以保留一个服务器故障：**是**。
- 一台服务器出现故障，则另一个可以经受得住: * * 的 Depends * * （不能经受如果这两个节点 3 和 4 下降，但可以经受得住所有其他方案。
- 可以在一次能够经受两个服务器故障: * * 的 Depends * * （不能经受如果这两个节点 3 和 4 下降，但可以经受得住所有其他方案。
