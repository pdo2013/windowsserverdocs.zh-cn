...
-   [身份验证策略思](how-to-configure-protected-accounts.md#BKMK_CreateAuthNPolicySilos)  
  
有其他缓解内置于 Windows 8.1 和 Windows Server 2012 R2 来帮助防止凭据被盗涵盖以下主题：  
  
-   [远程桌面版的限制的管理员模式](http://blogs.technet.com/b/kfalde/archive/20../restricted-admin-mode-for-rdp-in-windows-8-1-2012-r2.aspx)  
...
-   若要提供受保护的用户，该限制种身份验证的使用情况是，域控制器端限制以及其他限制，域功能级别必须是 Windows Server 2012 R2。 有关功能级别的详细信息，请参阅[了解 Active Directory 域服务 (广告 DS) 功能级别](../../identity/ad-ds/active-directory-functional-levels.md)。  
...
![显示如何域控制器设置 TGT 生命周期内和基于域策略续订组策略编辑器中管理窗口的屏幕截图](../media/how-to-configure-protected-accounts/ADDS_ProtectAcct_TGTExpiration.png)  
...
![显示要检查的位置的屏幕截图 * * 敏感帐户，不能委派 * * UI 元素](../media/how-to-configure-protected-accounts/ADDS_ProtectAcct_TshootDelegation.gif)  
...
-   在 Kerberos 预身份验证拒绝 RC4： 无法配置。  
    ...
    > [!NOTE]  
    > 尽管到可以[更改支持的加密类型配置](http://blogs.msdn.com/b/openspecification/archive/20../windows-configurations-for-kerberos-supported-encryption-type.aspx)，最好不要无需测试的目标环境中更改这些设置为计算机帐户。  
...
![显示协议交易所，也称为子 Kerberos 身份验证的三种类型的屏幕截图](../media/how-to-configure-protected-accounts/ADDS_ProtectAcct_KerbRefresher.gif)  
...
有关详细信息，请参阅 [Kerberos 5 版本身份验证协议工作原理] (https://technet.microsoft.com/library/cc772815(v=WS.10.aspx.  
...
![演示如何通过配置限制用户登录 TGT 生命周期内和访问控制条件限制初始身份验证的屏幕截图](../media/how-to-configure-protected-accounts/ADDS_ProtectAcct_RestrictAS.gif)  
...
1.  在默认域控制器策略中，单击**启用**启用**密钥 Distribution 中心 (KDC) 客户端支持索赔、 复合身份验证和 Kerberos 程度**配置计算机 |管理模板 |系统 |KDC。  
    ...
    ![在默认域控制器策略中，单击 * * 已启用 * * 启用 * * 密钥 Distribution 中心 (KDC) 客户端支持索赔、 复合身份验证和 Kerberos 程度 * * 配置计算机 |管理模板 |系统 |KDC](../media/how-to-configure-protected-accounts/ADDS_ProtectAcct_EnableKDCClaims.gif)  
...
2.  下**选项**的下拉列表中，选择**始终提供索赔**。  
    ...
    ![下 * * 选项 * *，在下拉列表中，选择 * * 始终提供索赔](../media/how-to-configure-protected-accounts/ADDS_ProtectAcct_AlwaysProvideClaims.png)  
...
1.  管理中心打开的 Active Directory (ADAC)。  
    ...
    ![显示 Active Directory 管理中心的屏幕截图](../media/how-to-configure-protected-accounts/ADDS_ProtectAcct_OpenADAC.gif)  
...
4.  若要配置为用户帐户 TGT 生命周期内，请选择**指定授权票证有效期为用户帐户**复选框，输入在几分钟内的时间。  
    ...
    ![指定授权票证有效期为用户帐户](../media/how-to-configure-protected-accounts/ADDS_ProtectAcct_TGTLifetime.gif)  
    ...
    ![对于使用默认设置域组策略编辑器中管理窗口](../media/how-to-configure-protected-accounts/ADDS_ProtectAcct_TGTExpiration.png)  
  
5.  若要限制选定设备上的用户帐户，请单击**编辑**来定义的设备所需的条件。  
  
    ![若要限制选定设备上的用户帐户，请单击 * * 编辑 * *](../media/how-to-configure-protected-accounts/ADDS_ProtectAcct_EditAuthNPolicy.gif)  
...
2.  单击**添加项目**。  
    ...
    ![添加的项目](../media/how-to-configure-protected-accounts/ADDS_ProtectAcct_AddCompAddItems.png)  
...
6.  单击确定，并创建任何其他计算机帐户条件。  
    ...
    ![单击确定，并创建任何其他条件计算机帐户](../media/how-to-configure-protected-accounts/ADDS_ProtectAcct_AddCompAddConditions.png)  
...
1.  若要配置计算机索赔，下拉列表以选择声明的组。  
    ...
    ![若要配置计算机索赔，下拉列表选择索赔组](../media/how-to-configure-protected-accounts/ADDS_ProtectAcct_CompClaim.gif)  
  
    才可用他们已预配林中索赔。  
  
2.  键入 OU 的名称，应限制为登录的用户帐户。  
  
    ![键入 OU 的名称，应限制为登录的用户帐户。](../media/how-to-configure-protected-accounts/ADDS_ProtectAcct_CompClaimOUName.gif)  
  
3.  完成后，然后单击确定，并框将显示所定义的条件。  
  
    ![完成单击确定](../media/how-to-configure-protected-accounts/ADDS_ProtectAcct_CompClaimComplete.gif)  
...
![演示如何到计算机的限制基于部门 (OU) 身份验证的屏幕截图](../media/how-to-configure-protected-accounts/ADDS_ProtectAcct_RestrictComputers.gif)  
  
此声明可用来限制用户登录到设备，请选择**用户**复选框。  
  
![通过选中选择登录到设备演示如何限制用户的屏幕截图 * * 用户 * * 复选框。  ](../media/how-to-configure-protected-accounts/ADDS_ProtectAcct_RestrictUsersComputers.gif)  
...
2.  选择**分配到此帐户的身份验证策略**复选框。  
    ...
    ![选择 * * 身份验证策略为此帐户 * * 复选框](../media/how-to-configure-protected-accounts/ADDS_ProtectAcct_UserPolicyAssign.gif)  
  
3.  然后选择要适用于用户身份验证策略。  
  
    ![选择要适用于用户的身份验证策略](../media/how-to-configure-protected-accounts/ADDS_ProtectAcct_UserPolicySelect.png)  
...
![显示如何使用组策略或本地组策略编辑器启用的屏幕截图 * * Kerberos 客户端支持索赔、 复合身份验证和 Kerberos 程度 * *](../media/how-to-configure-protected-accounts/ADDS_ProtectAcct_KerbClientDACSupport.gif)  
...
![身份验证策略显示直接在应用策略帐户中的帐户部分的屏幕截图](../media/how-to-configure-protected-accounts/ADDS_ProtectAcct_AccountsAssigned.gif)  
...
1.  打开**Active Directory 管理中心**，单击**身份验证**，右键单击**身份验证策略思**，单击**新建**，然后单击**身份验证策略思**。  
    ...
    ![打开 * * 主动目录管理中心 * *，单击 * * 身份验证 * *，右键单击 * * 身份验证策略思 * *，单击 * * 新建 * *，，然后单击 * * 身份验证策略思 * *](../media/how-to-configure-protected-accounts/ADDS_ProtectAcct_CreateNewAuthNPolicySilo.gif)  
  
2.  在**显示名称**，为思键入一个名称。 在**允许帐户**，单击**添加**、 键入的帐户名称，然后单击**确定**。 你可以指定用户、 计算机或服务的帐户。 然后指定是否使用单个策略为所有主体或单独策略每种类型的用户，以及的策略或策略的名称。  
  
    ![在 * * 显示名称 * *、 键入思的名称。 在 * * 允许帐户 * *，单击 * * 添加 * *，键入的帐户名称，然后单击 * * 确定 * *](../media/how-to-configure-protected-accounts/ADDS_ProtectAcct_NewAuthNPolicySiloDisplayName.gif)  
