...
LSA 包含本地安全机构服务器服务 (LSASS) 进程，可以验证用户的本地和远程登录，并强制本地安全策略。 Windows 8.1 操作系统提供为 LSA 来防止读取内存及注入代码非受保护的进程的额外保护。 这为 LSA 存储和管理的凭据提供了更高的安全性。 LSA 的受保护的进程设置可以配置在 Windows 8.1 中，但它不能在 Windows RT 8.1 配置。 将此设置与安全启动结合使用时，便可以实现附加保护，因为禁用 HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Control\Lsa 注册表项不起作用。
...
1.  签名验证
    ...
    用作驱动程序（例如智能卡驱动程序）的 LSA 插件需要使用 WHQL 认证进行签名。 有关详细信息，请参阅[WHQL 版本签名](https://msdn.microsoft.com/library/windows/hardware/ff553976%28v=vs.85%29.aspx)。
...
#### <a name="limitations-introduced-with-enabled-lsa-protection"></a>使用已启用 LSA 保护引入的限制

如果启用 LSA 保护，则不能调试自定义的 LSA 插件。
受保护的进程时，您不能将调试器附加到 LSASS。
一般情况下，是不支持调试正在运行的受保护的进程的方法。
...
在此之后，可能会看到这些事件在事件查看器：Microsoft-Windows-Codeintegrity/操作：
...
（有或没有安全启动或 UEFI） 运行 Windows 8.1 设备上，配置是可以通过执行在本部分中所述的过程。 对于运行 Windows RT 8.1 设备，始终会启用 lsass.exe 保护，并不能关闭它。
