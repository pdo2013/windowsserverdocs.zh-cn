...
| 见证类型  | 描述  | 要求和建议  |
| ---------    |---------        |---------                        |
| 磁盘见证     |  <ul><li> 存储群集数据库副本的专用 LUN</li><li> 对具有共享（非复制）存储的群集最有用</li>       |  <ul><li>LUN 的大小必须至少有 512 MB</li><li> 必须专用于群集而且不分配给群集角色</li><li> 必须包括在群集存储中并且通过存储验证测试</li><li> 不可以是群集共享卷 (CSV) 磁盘</li><li> 具有单个卷的基本磁盘</li><li> 不需要有驱动器号</li><li> 可以使用 NTFS 或 ReFS 格式化</li><li> 可以使用容错功能的硬件 RAID 有选择性地进行配置</li><li> 应从备份和防病毒扫描中排除</li><li> 磁盘见证服务器不支持使用存储空间直通</li>|
...
| 云见证     |  <ul><li>在 Azure blob 存储中存储的见证服务器文件</li><li> 在群集中的所有服务器都具有可靠的 Internet 连接时，建议使用。</li>      |  请参阅[部署的云见证](https://docs.microsoft.com/windows-server/failover-clustering/deploy-cloud-witness)。       |
...
- 启用存储空间直通后，群集只能支持两个节点故障。 对此进行更[池仲裁部分](../storage/storage-spaces/understand-quorum.md)
...
3. 在“选择仲裁配置选项”  页面上，选择三个配置选项之一，并且完成该选项的步骤。 配置仲裁设置之前，你可以查看你的选择。 有关选项的详细信息，请参阅[了解仲裁](#understanding-quorum)本主题中前面部分。
...
在向导运行并且**摘要**页出现后，如果你想要查看向导所执行的任务的报告中，选择**查看报告**。 最新的报告将保留在<em>systemroot</em> **\\群集\\报表**文件夹中的，名称**QuorumConfiguration.mht**。
