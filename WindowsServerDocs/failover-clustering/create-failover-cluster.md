---
title: 创建故障转移群集
description: 如何创建针对 Windows Server 2012 R2、 Windows Server 2012 和 Windows Server 2016 故障转移群集。
...
ms.locfileid: "59843288"
---
# <a name="create-a-failover-cluster"></a>创建故障转移群集

>适用于：Windows Server 2012 R2、 Windows Server 2012 中，Windows Server 2016

本主题介绍如何通过使用故障转移群集管理器管理单元或 Windows PowerShell 创建故障转移群集。 该主题包含一个典型的部署，其中群集及其相关联的群集角色的计算机对象在 Active Directory 域服务 (AD DS) 中创建。 如果要部署存储空间直通群集，而是请参阅[部署存储空间直通](../storage/storage-spaces/deploy-storage-spaces-direct.md)。

您还可以部署与 Active Directory 分离的群集。 通过此部署方法可创建故障转移群集，无需拥有在 AD DS 中创建计算机对象的权限，也无需请求在 AD DS 中预留计算机对象。 此选项仅通过 Windows PowerShell 提供，并且仅建议用于特定方案。 有关详细信息，请参阅[部署与 Active Directory 分离的群集](https://docs.microsoft.com/previous-versions/windows/it-pro/windows-server-2012-r2-and-2012/dn265970(v=ws.11))。
...
- 检查硬件要求，以确保你的配置受支持。 有关详细信息，请参阅 [Failover Clustering Hardware Requirements and Storage Options](https://docs.microsoft.com/previous-versions/windows/it-pro/windows-server-2012-r2-and-2012/jj612869(v%3dws.11))。 如果要创建存储空间直通群集，请参阅[存储空间直通的硬件要求](../storage/storage-spaces/storage-spaces-direct-hardware-requirements.md)。
- 若要在创建群集期间添加群集的存储，请确保所有服务器可以都访问该存储。 （你还可以在创建群集之后添加群集存储。）
...
1. 启动“服务器管理器”。
2. 上**管理**菜单中，选择**添加角色和功能**。
3. 上**在开始之前**页上，选择**下一步**。
4. 上**选择安装类型**页上，选择**基于角色或基于功能的安装**，然后选择**下一步**。
5. 上**选择目标服务器**页上，选择你想要安装该功能，然后选择的服务器**下一步**。
6. 上**选择服务器角色**页上，选择**下一步**。
...
1. 启动“服务器管理器”。
2. 上**工具**菜单中，选择**故障转移群集管理器**。
3. 在中**故障转移群集管理器**窗格下**管理**，选择**创建群集**。
...
7. 在“用于管理群集的访问点”页面上，执行以下操作：
    ...
    2. 如果该服务器没有配置为使用 DHCP 的网络适配器，则必须为该故障转移群集配置一个或多个静态 IP 地址。 选中你想要用于群集管理的每个网络旁边的复选框。 选择**地址**所选网络旁边的字段，然后输入你想要分配给群集的 IP 地址。 此 IP 地址（或多个地址）将与域名系统 (DNS) 中的群集名称相关联。
    3. 在完成后，请选择**下一步**。
...
9. 选择**下一步**创建故障转移群集。
10. 在“摘要”页面上，确认已成功创建故障转移群集。 如果没有任何警告或错误，查看摘要输出或 select**查看报表**若要查看完整报告。 选择**完成**。
11. 若要确认已创建群集，请验证该群集名称在导航树中的“故障转移群集管理器”下列出。 您可以展开群集名称，然后选择下的项**节点**，**存储**或**网络**以查看关联的资源。
    
    请注意，在 DNS 中成功复制群集名称可能需要花一些时间。 在成功的 DNS 注册并复制，如果您选择后**的所有服务器**在服务器管理器中，群集名称应为的服务器列出**可管理性**的状态**联机**.

创建该群集后，你可以执行以下操作：验证群集仲裁配置和（可选）创建群集共享卷 (CSV)。 有关详细信息，请参阅[存储空间直通中了解仲裁](../storage/storage-spaces/understand-quorum.md)并[使用故障转移群集中的群集共享卷](failover-cluster-csvs.md)。
...
4. 若要验证是否已创建群集角色，请在“角色”窗格中，确保该角色的状态为“运行”。 “角色”窗格中还指示所有者节点。 若要测试故障转移，请右键单击角色，指向**移动**，然后选择**选择节点**。 在中**移动群集角色**对话框中，选择所需的群集节点，并选择**确定**。 在“所有者节点”  列中，验证该所有者节点是否已更改。
...
## <a name="more-information"></a>详细信息
  ...
  - [部署的 HYPER-V 群集](<https://docs.microsoft.com/previous-versions/windows/it-pro/windows-server-2012-r2-and-2012/jj863389(v%3dws.11)>)
