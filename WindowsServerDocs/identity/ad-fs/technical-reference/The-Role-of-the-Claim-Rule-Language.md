...
例如，在下图中，传入声明由声明发出引擎添加到输入声明集。 当第一个自定义声明规则执行并满足条件的域用户，声明发出引擎处理中使用添加语句和的值的规则的逻辑**编辑器**添加到输入的声明集。 由于值 Editor 是存在于输入的声明集中，规则 2 可以成功处理其逻辑中的发出语句并生成一个新的值**Hello**，这添加到这两个输出声明集和输入声明集以供在规则中的下一个规则集。 规则 3 现在可以使用所有输入声明集作为输入来处理其逻辑中存在的值。  
