...
### <a name="critical-system-files"></a>关键系统文件  
 安装 Windows 操作系统时，安装程序将在系统驱动器上创建文件夹，它将在这些文件夹中放置系统需要启动并运行的文件。 关键系统文件包括具有 .dll、.exe、.ocx 和 .sys 文件扩展名的文件。 其中某些文件采用 True Type 字体。 此外，系统状态文件，例如系统的注册表中，所需操作系统才能正常运行。  
