...
5.  定义任务操作：  
    ...
    3.  在 "**添加参数**（可选）" 文本框中，键入以下内容（必须包含引号）： **set dsrm password sync From domain account SBS_network_administrator_account q q** ，其中*SBS_network_administrator_account*是当前网络管理员的帐户名称。  
...
5.  在 **< 文件夹\>名属性**中，单击 "**共享**"，为列出的用户帐户选择适当的用户访问级别，然后单击 "**应用**"。  
